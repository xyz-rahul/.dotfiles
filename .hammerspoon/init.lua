...
local function navigateTab(direction)
    ...
    if focusedApp and
        (focusedApp:name() == "Google Chrome" or focusedApp:name() == "Brave Browser" or focusedApp:name() ==
            "Microsoft Edge") then
...
local nextTabHotkey = nil
local prevTabHotkey = nil

function applicationWatcher(appName, eventType, appObject)
    if (eventType == hs.application.watcher.activated) then
        if (appName == "Google Chrome" or appName == "Brave Browser" or appName == "Microsoft Edge") then
            -- Bind Ctrl + N for next tab
            nextTabHotkey = hs.hotkey.bind({"ctrl"}, "n", function()
                navigateTab("next")
            end)

            -- Bind Ctrl + P for previous tab
            prevTabHotkey = hs.hotkey.bind({"ctrl"}, "p", function()
                navigateTab("prev")
            end)
        end
    end
    ...
    if (eventType == hs.application.watcher.deactivated) then
        if (appName == "Google Chrome" or appName == "Brave Browser" or appName == "Microsoft Edge") then
            -- Disable the custom keybindings
            if nextTabHotkey then
                nextTabHotkey:delete()
                nextTabHotkey = nil
            end
            if prevTabHotkey then
                prevTabHotkey:delete()
                prevTabHotkey = nil
            end
        end
    end
